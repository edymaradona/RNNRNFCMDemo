...
import HandleDeepLink from '../../src/notification/handleDeepLink'
class NavigationTypes extends React.Component {
  ...
  async onNavigatorEvent(event) {
    if (event.type === 'DeepLink') {
      ...
      const deeplink = new HandleDeepLink(parts, this.props.navigator, null);
      if (parts[0] === 'tab1') {
        this.props.navigator.push({
          ...
          const count = await notification.getInboxKeyCount(payload.inboxStyleKey);
          deeplink.navigate(count, payload);
      } else { deeplink.navigate(); }
